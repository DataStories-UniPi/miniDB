...
class Table:
    ...
    '''
    def __init__(self, name=None, column_names=None, column_types=None, columns_unique = None , primary_key=None, load=None):
        ...
        # if name, columns_names and column types are not none
            ...
            self.columns_unique = columns_unique if columns_unique is not None else []
    ...
    def _select_where(self, return_columns, condition=None, distinct=False, order_by=None, desc=True, limit=None):
        ...
        if condition is not None:
            if "between" in condition.split():
                ...
                if(split_con[3]=='and'):
                    low_value = split_con[2]
                    high_value = split_con[4]
                    column_name = split_con[0]
                    column = self.column_by_name(column_name)
                    rows = []

                    if(low_value.isdigit() and high_value.isdigit()):
                         x = int(low_value) 
                         y = int(high_value)
                        ...
                        print('Not integers, please try again and write the correct query')
    ...
    def _select_where_with_hash(elf, return_columns, hashh, condition, distinct=False, order_by=None, desc=True, limit=None):
        if return_columns == '*':
            return_cols = [i for i in range(len(self.column_names))]
        else:
            return_cols = [self.column_names.index(colname) for colname in return_columns]
        ...
        if operator == '<' or operator == '>':
            ...
            hash_ind = sum_of_hash % int(hashh[0][0][0])

            for item in hashh[hash_ind]:
                if hashh[hash_ind][item][0] == hashh[0][0][0]:
                    continue
                if hashh[hash_ind][item][1] == value:
                    rows.append(hashh[hash_ind][item][0])
        try:
            k = int(limit)
        except TypeError:
            k = None
