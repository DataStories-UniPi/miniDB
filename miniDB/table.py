...
class Table:
    ...
    def _select_hash(self, return_columns, condition=None, distinct=False, order_by=None, desc=True, limit=None):
        ...
        for i in range(0, len(pkcolumn)):
            h = int(pkcolumn[i]) % number_of_bins
        ...
        if str.isdigit(str(value)):
            ...
            for i in range(0, len(ht[h])):
                if str(ht[h][i]) == str(val):
                    # copy the old dict, but only the value given by the hash function
                    return self._build_dict([self.data[ptr[h][i]]], return_cols, distinct, order_by, desc, limit)

    def _build_dict(self, data, return_cols, distinct=False, order_by=None, desc=True, limit=None):
        dict = {key: (data if key == "data" else value)
                for
    ...
    def _select_where(self, return_columns, condition=None, distinct=False, order_by=None, desc=True, limit=None):
        ...
        return self._build_dict([[self.data[i][j] for j in return_cols] for i in rows], return_cols, distinct, order_by,
                                desc, limit)

    def _select_where_with_btree(self, return_columns, bt, condition, distinct=False, order_by=None, desc=True,
                                 limit=None):
        ...
        rows = rows[:k]
        return self._build_dict([[self.data[i][j] for j in return_cols] for i in rows],return_cols,distinct,order_by,desc,limit)
