...
class Database:
    ...
    def select(self, columns, table_name, condition, order_by=None, top_k=True, distinct=False, desc=None, save_as=None, return_object=True):
        '''
        ...
        if isinstance(table_name,Table):
            return table_name._select_where(columns, condition, order_by, distinct, desc, top_k)
        ...
        if self._has_index(table_name) and condition_column==self.tables[table_name].column_names[self.tables[table_name].pk_idx]:
            ...
            table = self.tables[table_name]._select_where_with_btree(columns, bt, condition, order_by, distinct, desc, top_k)
        else:
            table = self.tables[table_name]._select_where(columns, condition, order_by, distinct, desc, top_k)
        # self.unlock_table(table_name)
