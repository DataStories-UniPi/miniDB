...
class Database:
    ...
    def __init__(self, name, load=True):
        self.tables = {}
        self._name = name
        # we create variables that have being using for the trigger function
        self.tb_name = name
        self.tb2_name = name
        self.str_con = name
        self.action = name
    ...
    def create_trigger(self, test, t1, t2, condition):
        """
        Create trigger that is going to execute an insert into a table after the insert in a table
        Args:
    		test: has the all the sentence that the user gave for the trigger function
            t1: variable that is being used for the function to take values
            t2: variable that is being used for the function to take values
            condition: variable that is being used for the function to take values and will be used to check the
            condition of the args that user gives

        """
        test1 = test.split()
        # variables from self that can to every def
    ...
    def insert_into(self, table_name, row_str):
        ...
        Args:
            ...
            lock_load_save: boolean. If False, user needs to load, lock and save the states of the database (CAUTION).
            Useful for bulk-loading.
        '''
        table_n2 = self.tb2_name
        con = self.str_con

        if self.action == 'before':# if for trigger function. they being used for knowing which action need to happend

            if table_name == self.tb_name: # to check if it's the right table
                print("you are in the trigger function")
                self.delete_from(table_n2, con)# calling delete action
                self.action = ''
                self.tb_name = ''
            # from here we have the insert code that has not being changes from as
            ...
            self.save_database()
            #till here
        elif self.action == "instead":# if for trigger function. they being used for knowing which action need to happend
            ...
            if table_name == self.tb_name:# to check if it's the right table
                ...
                self.save_database()
                #till here
        elif self.action == 'after':# if for trigger function. they being used for knowing which action need to happend
            # from here we have the insert code that has not being changes from as
