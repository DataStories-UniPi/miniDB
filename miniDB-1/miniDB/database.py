...
class Database:
    ...
    def create_table(self, name, column_names, column_types, columns_unique = None, primary_key=None, load=None):
        ...
        Args:
            ...
            colums_unique :list. Names of columns with the UNIQUE constraint.
            primary_key: string. The primary key (if it exists).
            load: boolean. Defines table object parameters as the name of the table and the column names.
        '''
        # print('here -> ', column_names.split(','))
        self.tables.update({name: Table(name=name, column_names=column_names.split(','), column_types=column_types.split(','), primary_key=primary_key, load=load, columns_unique = columns_unique.split(',') if columns_unique is not None else None)})
    ...
    def join(self, mode, left_table, right_table, condition, save_as=None, return_object=True):
        ...
        if mode=='inner':
            ...
            if not leftIndexExists and not rightIndexExists:
                ...
                index_name = \
                     self.select('*', 'meta_indexes', f'table_name={right_table._name}', return_object=True).column_by_name('index_name')[0]
                res = Inlj(condition, left_table, right_table, self._load_idx(index_name), 'right').join()
            elif leftIndexExists:
                index_name = \
                     self.select('*', 'meta_indexes', f'table_name={left_table._name}', return_object=True).column_by_name('index_name')[0]
    ...
    def create_index(self, index_name, table_name, column_name , index_type):
        ...
        if table_name not in self.tables:
            raise Exception(" You cant create index beucause  the table does not exist. Please write the correct  table name") 
    ...
    def _construct_hash_index(self, table_name, index_name , column_name):
        row_length = len(self.tables[table_name].data)
        hashh = {}
        hashh[0] = {}
        hash[0][0] =[str(row_length)]
        for idx, key in enumerate(self.tables[table_name].column_by_name(column_name)):
            if key is None:
                continue
            sum_of_hash = 0
            ...
            hash_ind = sum_of_hash % row_length
            sub_hash_ind = hash_ind
            if not(hash_ind in hashh):
                hashh[hash_ind] = {}
            else:
                while True:
                    if not(sub_hash_ind in hashh[hash_ind]):
                        break
                    if sub_hash_ind == row_length:
                        sub_hash_ind = 0
                    else:
                        sub_hash_ind += 1
            hashh[hash_ind][sub_hash_ind] = [idx,key]

        self._save_index(index_name, hashh)
