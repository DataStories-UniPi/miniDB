...
def evaluate_from_clause(dic):
    ...
    '''
    join_types = ['inlj','inner', 'left', 'right', 'full']
    ...
    if fname is not None:
        ...
        while 1:
            try:
                ...
                Database.clear_tempviews(db) #Calling function from Database class to clear temp views when app interrupted
                print('\nbye!')
                break
            try:
                if line == 'exit;':
                    Database.clear_tempviews(db) #Calling function from Database class to clear temp views when app exiting
                    break
                if line.startswith('.'):
                    interpret_meta(line)
