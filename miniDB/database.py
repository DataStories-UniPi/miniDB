...
class Database:
    ...
    def insert_into(self, table_name, row_str,columns=None,table_nam=None,condition=None):
        '''
        ...
        '''
        if table_nam is not None:
            if isinstance(table_nam, Table):
                table = table_nam._select_where(columns,condition)
            else :
                table = self.tables[table_nam]._select_where(columns,condition)
        
        if columns is None and row_str is not None:
    ...
    def select(self, columns, table_name, condition, in_args=None, between=None, like=None, order_by=None, top_k=True,
        ...
        if isinstance(table_name, Table):
            return table_name._select_where(columns, condition, in_args, between, like, order_by, desc, top_k)

        if condition is not None:
            if in_args is not None:
                condition_column = condition
                in_args = in_args.strip('(').strip(')').split(',')
        ...
        if self._has_index(table_name) and condition_column == self.tables[table_name].column_names[self.tables[table_name].pk_idx]:
            ...
            table = self.tables[table_name]._select_where_with_btree(
                columns, bt, condition, in_args, between, like, order_by, desc, top_k)
        else:
            table = self.tables[table_name]._select_where(
                columns, condition, in_args, between, like, order_by, desc, top_k)
        # self.unlock_table(table_name)
    ...
    def journal(idx=None):
        if idx != None:
            cache_list = '\n'.join([str(readline.get_history_item(
                i + 1)) for i in range(readline.get_current_history_length())]).split('\n')[int(idx)]
            out = tabulate({"Command": cache_list.split('\n')},
                           headers=["Command"])
        else:
            cache_list = '\n'.join([str(readline.get_history_item(
                i + 1)) for i in range(readline.get_current_history_length())])
            out = tabulate({"Command": cache_list.split('\n')}, headers=[
                           "Index", "Command"], showindex="always")
        print('journal:', out)
        # return out
