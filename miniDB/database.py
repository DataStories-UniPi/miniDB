...
class Database:
    ...
    def create_table(self, name, column_names, column_types, primary_key=None, unique=None, load=None):
        ...
        self.tables.update({name: Table(name=name, column_names=column_names.split(','),
                                        column_types=column_types.split(','), primary_key=primary_key, unique=unique,
                                        load=load)})
    ...
    def select(self, columns, table_name, condition, distinct=None, order_by=None,
        ...
        if self.tables[table_name].pk_idx is not None and condition_column==self.tables[table_name].column_names[self.tables[table_name].pk_idx]:
            ...
            if operator== "=":
                return self.tables[table_name]._select_hash(columns, condition, distinct, order_by, desc, limit)
        ...
        if self._has_index(table_name) and condition_column == self.tables[table_name].column_names[
            ...
            table = self.tables[table_name]._select_where_with_btree(columns, bt, condition, distinct, order_by, desc,
                                                                     limit)
        elif (self._has_index(table_name) and condition_column == self.tables[table_name].column_names[
            self.tables[table_name].unique_idx]):
            index_name = \
                self.select('*', 'meta_indexes', f'table_name={table_name}', return_object=True).column_by_name(
                    'index_name')[0]
            bt = self._load_idx(index_name)
            ...
            if not leftIndexExists and not rightIndexExists:
                ...
                index_name = \
                    self.select('*', 'meta_indexes', f'table_name={right_table._name}',
                                return_object=True).column_by_name(
                        'index_name')[0]
